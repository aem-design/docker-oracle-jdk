Param(
  ...
  [string]$COMMAND = "docker build --build-arg ORACLE_PASSWORD=$ORACLE_PASSWORD . -f .\${FILE} -t ${TAG}"
)
...
printSectionLine ( $COMMAND -replace $ORACLE_PASSWORD, $( "*" * $ORACLE_PASSWORD.length ) ) "warn"
